...
v3dell)
        ...
        for ii in range(len(self.evalells)):
            ...
            self.N_ll_rsxEE[ii] = np.dot(np.transpose(self.W_ll_rsxEE[ii,:]),np.dot(N_ll_for_rsxEE,self.W_ll_rsxEE[ii,:]))
    ...
    def Forecast_CellrsxEE(self,ellBinEdges,fsky,option='None'):
        ...
        LF = LensForecast()
        if (option=='None'):        
            LF.loadGenericCls("ee",self.evalells,cls_rsxEE,self.evalells,self.N_ll_rsxEE)
        elif (option=='NoILC'):
            LF.loadGenericCls("ee",self.evalells,cls_rsxEE,self.evalells,self.N_ll_rsx_NoFG)
        elif(option=='Onlyfg'):
            LF.loadGenericCls("ee",self.evalells,cls_rsxEE,self.evalells,self.N_ll_rsxEE-self.N_ll_rsx_NoFG)
        else:
            return "Wrong option"
