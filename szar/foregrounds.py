...
class fgNoises(object):
    ...
    beta_d = 1.59
    beta_s=-3.1
    def pl_dust_power(self,freq_in_GHz, nu_353, beta_d=1.59):
        nu = freq_in_GHz
        T_dust = 19.6
        return ((nu/(nu_353))**(beta_d-2)*old_div(self.B_nu(T_dust,nu),self.B_nu(T_dust,353.)))**2

    def sync_power(self,freq_in_GHz, nu_K, beta_s=-3.1):
        nu = freq_in_GHz
    ...
    def gal_dust_pol(self,ell,nu1,nu2):
        ...
        #mu0 = pol.pl_dust_power(self.c['nu0'])
        ans = self.c['A_gal_dust'] * (old_div(ell,self.c['ell0sec'])) ** self.c['alpha_gd'] * mu1 * mu2 #/ mu0**2
        return ans
